 ...
 * caso de uso e a gravação. O índice que ela cria começa por `propriedade_id`, e por isso
 * serve tanto ao filtro quanto à ordem da listagem, que é por Cultura e depois por Safra.
 * Não há índice separado para isso.
 *
 * `cultura_id` e `safra_id` recebem índice próprio porque o painel conta os Plantios por
 * Cultura e a listagem do painel recorta por Safra. Ver o registro 0004.
 */
export class CriaPlantios1789080000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE INDEX "ix_plantios_safra" ON "plantios" ("safra_id")`);
  }
